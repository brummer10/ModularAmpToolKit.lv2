...
#define PLUGIN_URI "urn:brummer:PreAmpImpulses"
...
// constructor
Xpreampimpulses::Xpreampimpulses() :
...
void Xpreampimpulses::init_dsp_(uint32_t rate, uint32_t bufsize_)
{
    plugin1->init(rate);
    plugin2->init(rate);
    s_rate = rate;
    ...
    bufsize = bufsize_;
    if (bufsize ) {
        PreDesc& pre = *getPreEntry(static_cast<uint32_t>(tube_style_)).data;
...
void Xpreampimpulses::connect_(uint32_t port,void* data)
{
...
void Xpreampimpulses::activate_f()
{
    // allocate the internal DSP mem
}

void Xpreampimpulses::clean_up()
{
    // delete the internal DSP mem
}

void Xpreampimpulses::deactivate_f()
{
    // delete the internal DSP mem
}

float* Xpreampimpulses::adjust(float *buf, int32_t size, int set) {
    delete [] buffer;
    buffer = NULL;
...
void Xpreampimpulses::do_work_mono()
{
...
void Xpreampimpulses::run_dsp_(uint32_t n_samples)
{
...
void Xpreampimpulses::connect_all__ports(uint32_t port, void* data)
{
    // connect the Ports used by the plug-in class
    connect_(port,data);
...
LV2_Handle 
Xpreampimpulses::instantiate(const LV2_Descriptor* descriptor,
                            double rate, const char* bundle_path,
                            const LV2_Feature* const* features)
{
    // init the plug-in class
    Xpreampimpulses *self = new Xpreampimpulses();
    if (!self) {
        return NULL;
    }
...
void Xpreampimpulses::connect_port(LV2_Handle instance, 
                                   uint32_t port, void* data)
{
    // connect all ports
    static_cast<Xpreampimpulses*>(instance)->connect_all__ports(port, data);
}

void Xpreampimpulses::activate(LV2_Handle instance)
{
    // allocate needed mem
    static_cast<Xpreampimpulses*>(instance)->activate_f();
}

void Xpreampimpulses::run(LV2_Handle instance, uint32_t n_samples)
{
    // run dsp
    static_cast<Xpreampimpulses*>(instance)->run_dsp_(n_samples);
}

void Xpreampimpulses::deactivate(LV2_Handle instance)
{
    // free allocated mem
    static_cast<Xpreampimpulses*>(instance)->deactivate_f();
}

void Xpreampimpulses::cleanup(LV2_Handle instance)
{
    // well, clean up after us
    Xpreampimpulses* self = static_cast<Xpreampimpulses*>(instance);
    self->clean_up();
    delete self;
}

LV2_Worker_Status Xpreampimpulses::work(LV2_Handle instance,
     LV2_Worker_Respond_Function respond,
     LV2_Worker_Respond_Handle   handle,
     uint32_t                    size,
     const void*                 data)
{
  static_cast<Xpreampimpulses*>(instance)->do_work_mono();
  return LV2_WORKER_SUCCESS;
}

LV2_Worker_Status Xpreampimpulses::work_response(LV2_Handle instance,
              uint32_t    size,
              const void* data)
{
...
const void* Xpreampimpulses::extension_data(const char* uri)
{
...
const LV2_Descriptor Xpreampimpulses::descriptor =
{
    PLUGIN_URI ,
    Xpreampimpulses::instantiate,
    Xpreampimpulses::connect_port,
    Xpreampimpulses::activate,
    Xpreampimpulses::run,
    Xpreampimpulses::deactivate,
    Xpreampimpulses::cleanup,
    Xpreampimpulses::extension_data
};
...
lv2_descriptor(uint32_t index)
{
    ...
    {
        case 0:
            return &preampimpulses::Xpreampimpulses::descriptor;
        default:
            return NULL;
    }
}
