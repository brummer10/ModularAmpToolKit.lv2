...
#define PLUGIN_UI_URI "urn:brummer:PreAmps_ui"
...
const char* plugin_set_name() {
    return "PreAmps"; //set plugin name to display on UI
}
...
void plugin_create_controller_widgets(X11_UI *ui, const char * plugin_uri, float scale) {
    ...
    ui->widget[3] = add_lv2_combobox (ui->widget[3], ui->win, 5, "AmpSelect", ui, 319,  81, 120 * scale, 30 * scale);
    combobox_add_entry (ui->widget[3], "12ax7");
    combobox_add_entry (ui->widget[3], "12au7");
    combobox_add_entry (ui->widget[3], "12at7");
    combobox_add_entry (ui->widget[3], "6DJ8");
    set_adjustment(ui->widget[3]->adj, 0.0, 0.0, 0.0, 3.0, 1.0, CL_ENUM);
